"""Complete backtest + chart generation for all 4 backtests.
US/India Stocks: load from historical_screener (fast, ~700K rows).
US/India Strings: skip for now (75M rows too slow)."""
...
def load_stock_data(db_file):
    conn = sqlite3.connect(db_file, timeout=30)
    conn.execute('PRAGMA busy_timeout=60000')
    conn.execute('PRAGMA journal_mode=WAL')
    print('  Loading...', flush=True)
    df = pd.read_sql_query(
        'SELECT symbol, date, price, next_day_return, prob_up_st_cross '
        'FROM historical_screener '
        'WHERE atr_crossed_above = 1 AND next_day_return IS NOT NULL ORDER BY date',
        conn
    )
    conn.close()
    df['next_day_return'] = df['next_day_return'] / 100.0  # stored as pct, convert to decimal
    df['next_day_return'] = df['next_day_return'].clip(-0.5, 2.0)
    print('  Total: ' + str(len(df)) + ' rows, ' + str(df['symbol'].nunique()) + ' symbols', flush=True)
    return df

def run_bt(df, top_n=TOP_N):
    df = df.sort_values(['date', 'prob_up_st_cross'], ascending=[True, False])
    top = df.groupby('date').head(top_n)
    daily = top.groupby('date')['next_day_return'].mean().reset_index()
    daily.columns = ['date', 'port_ret']
    daily = daily.sort_values('date')
    return daily['port_ret'].values.astype(float), daily['date'].tolist()
...
def generate_charts(bt_name, ret, dates_list, all_period_stats, df_raw=None):
    dates_dt = pd.to_datetime(dates_list)
    ...
    table_data = [
        ...
        ['Unique Symbols', full_stats.get('Unique Symbols', 'N/A') if 'Unique Symbols' in full_stats else 'N/A'],
    ]
...
if __name__ == '__main__':
    ...
    for bname, dbf in [('US Stocks', 'screener.db'), ('India Stocks', 'india.db')]:
        print('\n=== ' + bname + ' ===', flush=True)
        df = load_stock_data(dbf)
        if df.empty: continue
        ret, dates_list = run_bt(df)
    ...
    print('\nSaved backtest_results.json', flush=True)
