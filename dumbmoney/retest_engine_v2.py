...
# ---------------------------------------------------------------------------
# Model loading and scoring (reuses V1's CatBoost model + feature extraction)
# ---------------------------------------------------------------------------
_V2_MODEL = None
_MODEL_PATH = os.path.join(os.path.dirname(os.path.dirname(__file__)), "models", "retest_v1", "model.cbm")


def _event_to_feature_array(ev, zones):
    """Extract 29-feature numpy array from a V2Event for model prediction.

    Maps V2 attribute names to the V1-trained CatBoost model's expected feature
    order. Missing V2 attributes use sensible defaults derived from V1 feature
    distributions.
    """
    zone_info = None
    for z in zones:
        if z.id == ev.zone_id:
            zone_info = {
                ...
                "width": 0.5,  # V2 zones don't track width; default from V1 dist
    ...
    return np.array([[
        nan_to(ev.breakout_body_atr, 0.0),
        nan_to(ev.breakout_close_location, 0.5),
        nan_to(ev.breakout_gap_atr, 0.0),
        nan_to(ev.breakout_volume_ratio, 1.0),
        0.0,  # breakout_consecutive_closes: not tracked in V2
        nan_to(ev.breakout_prior_close_rel, 0.0),
        0.0,  # breakout_retreat_within_3: not tracked in V2
        float(ev.age_at_breakout) if hasattr(ev, 'age_at_breakout') else 0.0,
        0.0,  # retest_low_atr: not tracked in V2
        nan_to(ev.pullback_from_peak_atr, 0.0),  # maps to retest_depth_atr
        float(ev.bars_peak_to_touch) if hasattr(ev, 'bars_peak_to_touch') else 0.0,  # maps to retest_touch_candles
        float(ev.return_closes_below_count) if hasattr(ev, 'return_closes_below_count') else 0.0,  # maps to retest_closes_below_level
        1.0,  # retest_volume_ratio: not tracked in V2; default from V1 dist
        ...
        float(ev.age_band) if hasattr(ev, 'age_band') else 0.0,
        0.0,  # trend_higher_highs: not tracked in V2
        0.0,  # context_pivot_low_dist_atr: not tracked in V2
        0.0,  # sma20_slope_atr: not tracked in V2
        0.0,  # sma20_above_sma60: not tracked in V2
        0.0,  # median_traded_value: not tracked in V2
        float(ev.entry) if hasattr(ev, 'entry') else 0.0,
        float(ev.signal_atr) if hasattr(ev, 'signal_atr') else 1.0,
        nan_to(ev.confirm_close_location, 0.5),
...
def load_model(path=None):
    ...
    try:
        ...
        _V2_MODEL = CatBoostClassifier()
        _V2_MODEL.load_model(path)
        logger.info(f"V2: Loaded retest model from {path} ({_V2_MODEL.tree_count_} trees)")
    except ImportError:
...
def check_model_drift():
    ...
    from dumbmoney.retest_engine import check_model_drift as _v1_drift
    return _v1_drift()


def populate_historical_scores(market, conn, only_symbols=None, progress_callback=None):
    """Bulk-populate historical_screener.old_swing_retest_score using V2 engine."""
    import pandas as pd
    c = conn.cursor()
    ...
    logger.info(f"V2: Populating historical retest scores for {total} symbols in {market}")
    ...
    for i, sym in enumerate(symbols):
        ...
        try:
            series = compute_retest_score_for_symbol_v2(df)
        except Exception:
            skipped += 1
            continue
        ...
        if progress_callback and (i + 1) % 50 == 0:
            progress_callback(i + 1, total, f"V2 historical_retest: {i+1}/{total}, {written} written")

    conn.commit()
    logger.info(f"V2 historical retest score population complete: {written} written, {skipped} skipped")
...
def compute_retest_score_current_v2(grp, model=None):
    ...
    loaded_model = model or _V2_MODEL
    score_fn = None
    if loaded_model is not None:
        try:
            score_fn = _make_score_fn(loaded_model)
        except Exception:
            pass

    try:
        result = fold_symbol_v2(h, l, c, o, v, dates, market, symbol, score_fn=score_fn)
    except Exception:
        return np.nan
    ...
    # Return the LAST non-NaN score (retest score is only set at confirmation,
    # NaN after — the "current" score is the most recent active signal score)
    for i in range(len(result.current_scores) - 1, -1, -1):
        val = result.current_scores[i]
