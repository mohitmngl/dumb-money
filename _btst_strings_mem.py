"""Load strings data in one shot, only needed columns, then simulate in-memory"""
import numpy as np, pandas as pd, sys, io, os, time
...
print("Loading minimal columns...", flush=True)
t0 = time.time()
df = pd.read_sql("""
    SELECT date, next_day_return, atr_crossed_above, prob_up_st_cross,
           weighted_alpha, streak, atr_streak, atr_signal, accel_signal,
           ai_volume_profile_score
...
# Precompute top-N per date for each strategy
strategies = [
